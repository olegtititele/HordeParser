...
namespace PostgreSQL
{
    public static class DB
    {
        ...
        public static void StartParsers(TelegramBotClient botClient)
        {
            foreach(var userId in GetAllUsersId())
            {
                try
                {
                    ...
                    {
                        string platform = DB.GetPlatform(userId);
                        Platforms.GetParser(botClient, userId, platform);
                    }
                }
                catch{ }
            }
        ...
        public static void CreateUsersTable()
        {
            ...
            cmd.CommandText = $"CREATE TABLE IF NOT EXISTS users_table (user_id BIGINT PRIMARY KEY, username TEXT, start_page INT, seller_type TEXT, whatsapp_text TEXT, state TEXT, parser TEXT, link TEXT, parser_category TEXT, seller_total_ads INT, seller_reg_date TEXT, seller_rating REAL, seller_feedback TEXT, platform TEXT, blacklist TEXT, timeout INT)";
            cmd.ExecuteNonQuery();
            con.Close();
        }
        ...
        public static void CreateUser(long userId, string username)
        {
            ...
            using var cmd = new NpgsqlCommand();
            cmd.Connection = con;
            cmd.CommandText = $"INSERT INTO users_table (user_id, username, start_page, seller_type, whatsapp_text, state, parser, link, seller_total_ads, seller_reg_date, seller_rating, seller_feedback, platform, blacklist, timeout) VALUES (@user_id, @username, @start_page, @seller_type, @whatsapp_text, @state, @parser, @link, @seller_total_ads, @seller_reg_date, @seller_rating, @seller_feedback, @platform, @blacklist, @timeout)";
            ...
            cmd.Parameters.AddWithValue("@seller_feedback", sellerFeedback.ToString());
        ...
        public static string GetSellerFeedback(long user_id)
        {
            using var con = new NpgsqlConnection(db_connection);
            con.Open();

            var sql = $"SELECT seller_feedback FROM users_table WHERE user_id = '{user_id}'";
            using var cmd = new NpgsqlCommand(sql, con);
            var result = cmd.ExecuteScalar();
            return result!.ToString()!;
        }
        ...
        public static void UpdateSellerFeedback(long userId, string sellerFeedback)
        {
            using (var conn = new NpgsqlConnection(db_connection))
            {
                ...
                using (var command = new NpgsqlCommand("UPDATE users_table SET seller_feedback = @q WHERE user_id = @n", conn))
                {
                    command.Parameters.AddWithValue("n", userId);
                    command.Parameters.AddWithValue("q", sellerFeedback);
                    int nRows = command.ExecuteNonQuery();
                }
            }
        }

        public static int GetSellerTotalAds(long user_id)
        {
            using var con = new NpgsqlConnection(db_connection);
            con.Open();
        ...
        public static void UpdateSellerTotalAds(long userId, int sellerAdv)
        {
            using (var conn = new NpgsqlConnection(db_connection))
            {
        ...
        public static int BlacklistLength(long user_id)
        {
            using var con = new NpgsqlConnection(blacklist_db);
            con.Open();
            var sql = $"SELECT DISTINCT seller_link FROM blacklist{user_id}";
            using var cmd = new NpgsqlCommand(sql, con);
        ...
        public static bool CheckSellerlink(long user_id, string seller_link)
        {
            try
            {
                ...
                var sql = $"SELECT * FROM blacklist{user_id} WHERE seller_link = '{seller_link}'";
                using var cmd = new NpgsqlCommand(sql, con);
                var result = cmd.ExecuteScalar()!.ToString();
        ...
        public static void DeleteSeller(long user_id, string seller)
        {
            using (var conn = new NpgsqlConnection(db_connection))
            {
                conn.Open();
                using (var command = new NpgsqlCommand($"DELETE FROM blacklist{user_id} WHERE seller_link = @q", conn))
                {
                    command.Parameters.AddWithValue("q", seller);
                    int nRows = command.ExecuteNonQuery();
                }
            }
        }

        public static List<string> GetAllBlSellers(long user_id)
        {
            List<string> sellers = new List<string>();
            using var con = new NpgsqlConnection(db_connection);
            con.Open();
            var sql = $"SELECT seller_link FROM blacklist{user_id}";
            using var cmd = new NpgsqlCommand(sql, con);
            cmd.ExecuteNonQuery();
            using (NpgsqlDataReader reader = cmd.ExecuteReader())
            {
